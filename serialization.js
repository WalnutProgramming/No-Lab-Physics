...
p5.Vector.prototype.toJSON = function() {
	return {x: this.x, y: this.y, $className: 'Vector'}
}
// Vector deserialization
p5.Vector.fromJSON = ({x, y}) => createVector(x, y)

/** The classes that can be serialized and deserialized */
const classes = {Vector: p5.Vector, Mover, BoxMover, CircleMover, Ruler, Draggable}
...
function serialize(state) {
	return JSON.stringify(
		...
		(_key, value) => {
			...
			if (typeof value === 'object' && value?.constructor?.name && Object.keys(classes).includes(value.constructor.name)) {
				return {...value, $className: value.constructor.name};
			}
			if (typeof value === 'object' && value != null && !['Array', 'Object'].includes(value.constructor.name)) {
				console.warn(
